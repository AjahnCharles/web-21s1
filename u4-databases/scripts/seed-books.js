...
// Fake search data (I don't recommend this code!!)
const getTerms = (book) => {
  return [
    ...book.authors.flatMap(author => author.split(' ').map(word => word.toLowerCase())), // names
    ...book.title.replace(/[\W_]+/g, ' ').split(' ').map(word => word.toLowerCase()) // all words in title
  ]
}

const booksWithSearch = books.map(book => ({
  ...book,
  searchTerms: getTerms(book)
}))

// Script
const main = async () => {
  // Start a write-only batch
  const batch = db.batch()

  // Call set once per book
  const booksRef = db.collection('books')
  booksWithSearch.forEach(book => {
    const bookRef = booksRef.doc(book.isbn13)
    batch.set(bookRef, book, { merge: true })
  })

  // Save data to Firestore
  await batch.commit()
  console.log(`Saved ${books.length} books to ${firebase.app().options.projectId}`)
  process.exit(0)
}
